...
**Decision**: Ship `omakase status` (read-only: approval, Stop conditions, next eligible item, exit codes for runners). The loop **motor** stays BYO — `omakase loop` remains deferred; ledger and queue writes stay with the agent.
