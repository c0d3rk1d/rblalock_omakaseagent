...
**Approval:** UNAPPROVED — a human replaces this line with "Approved by <name> on <date>" before any unattended run. Agents halt on UNAPPROVED and must never edit this line themselves.
...
Start with \`npx omakaseagent status\` — it evaluates approval, the charter's mechanical
Stop conditions, and the next eligible item; trust it over hand-parsing (plan-level
STOP rules and drift checks stay with the agent). Then one item per iteration via
...
Append rows at the **bottom** only. \`EMPTY\` must be the last row when the queue drains.
